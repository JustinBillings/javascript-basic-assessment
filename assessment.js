...
function sLoopers() {
	...
	for (var prop in snake) {
		var holder = snake[prop];
		if (snake[prop].charAt(0) === "s") {
			holder = "s";
		}
...
function getValueOfProperty(obj, keyName) {
	return obj[keyName];
}
...
function makeChatMessage(message, author) {
	 ...
	 var chatMessage = new makeChatMessge(message, author, timestamp);
...
function outside(temperature, humidity, cloudiness) {
	...
	else if (temperature > 80 && humidty < 40 && cloudiness < 20) {
